...
public abstract class ChoiceDataFactory extends AbstractChoiceSelectionFactory {
     ...
     * @throws Exception
     */
    public List<SelectableChoiceBean> customInitSelectChoice(String params) throws Exception {
        ArrayList<SelectableChoiceBean> list = new ArrayList<>();
        List<OptionInfoBean> defaultSelect = this.getOptionInfoBeansDefaultSelect(params);
        for (OptionInfoBean infoBean : defaultSelect) {
            list.addAll(infoBean.getSelectedChoices());
        }
        return list;
    }

    /**
     * 获取选项Bean对象以及所有的选择 默认设置对于的勾选的选择
     * 
     * @return
     * @throws Exception
     */
    public List<OptionInfoBean> getOptionInfoBeansDefaultSelect(String params) throws Exception {
        Collection choices = this.getChoices();
        RulesEngineSession ruleEngineSesssion = ChoiceComponentHelper.getRuleEngineSesssion(this.getFormData());
        ArrayList<OptionInfoBean> list = new ArrayList();
        TableDataManagerBean tableDataManagerBean = TableDataManagerBean.newInstance(this.getFormData());
        String tableId = ChoiceComponentHelper.getTableId(this.getFormData());
        boolean rulesEnabled = ChoiceComponentHelper.isRulesEnabled(this.getFormData());
        WTContainerRef wtContainerRef = ChoiceComponentHelper.getWTContainerRef(this.getFormData());
        WTHashSet set = new WTHashSet();
        String panzerVersion = this.getFormData().getTextParameter(params);
        ChoiceInfoBean choiceInfoBean;
        OptionInfoBean optionInfoBean;
        for (Object choice : choices) {
            choiceInfoBean = (ChoiceInfoBean)choice;
            // 如果当前选择为空或者是 则直接返回空值
            if (choiceInfoBean == null || choiceInfoBean.getOptionId() == null) {
                this.setChoiceNotAccessible(true);
                list.clear();
                break;
            }
            String optionId = choiceInfoBean.getOptionId();
            WTReference reference = this.getReferenceFactory().getReference(optionId);
            NmOid nmOid = new NmOid();
            nmOid.setWtRef(reference);
            if (!list.contains(nmOid)) {
                optionInfoBean = new OptionInfoBean(choiceInfoBean);
                optionInfoBean.setAllSelected(true);
                list.add(optionInfoBean);
                set.add(optionInfoBean.getWtRef());
            }
            optionInfoBean = (OptionInfoBean)list.get(list.indexOf(nmOid));
            Boolean isSelect = null;
            if (ruleEngineSesssion != null && rulesEnabled) {
                isSelect = this.isChecked(choiceInfoBean, ruleEngineSesssion);
            } else if (choiceInfoBean.getChoiceName().contains(panzerVersion)) {
                // 判断选择参数是否是90mm 如果是则直接默认选择
                isSelect = true;
            }

            // 这种应该是自己选择
            if (isSelect == null) {
                WTReference wtReference = this.getReferenceFactory().getReference(choiceInfoBean.getChoiceId());
                String choiceOid =
                    wtReference != null ? this.getReferenceFactory().getReferenceString(wtReference) : "";
                isSelect = this.isChecked(choiceOid, nmOid, tableDataManagerBean, tableId);
            }
            if (isSelect) {
                SelectableChoiceBean choiceBean = new SelectableChoiceBean(choiceInfoBean, wtContainerRef);
                choiceBean.setChoiceSelectionStatus(ChoiceSelectionStatus.CHECKED);
                if (this.isInitiallySelected() && choiceInfoBean.isExcludeEnabled()) {
                    choiceBean.setExcludeStatus(choiceInfoBean.getExcludeStatus());
                }
                optionInfoBean.addSelectedChoices(choiceBean);
            }
            if (choiceInfoBean.isExcludeEnabled()) {
                optionInfoBean.setExcludeChoicesEnabled(true);
            }
            if (CheckedStatus.CHECKED.equals(choiceInfoBean.getExcludeStatus())) {
                optionInfoBean.setExcludeChoices(true);
            }
            optionInfoBean
                .addChoiceDisplayInfos(ChoiceDisplayInfo.newChoiceDisplayInfo(choiceInfoBean, wtContainerRef));
        }
