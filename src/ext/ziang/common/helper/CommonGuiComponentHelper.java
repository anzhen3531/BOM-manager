...
public class CommonGuiComponentHelper {
     ...
     * @param isRequired        是必需
     * @param isReadOnly        是只读
     * @param isEditable        是可编辑
     * @param isEnabled         已启用
     * @param minSearchCharNum  最大长度
     * @param suggestServiceKey 建议服务密钥
     * @param paramsMap         Params 地图
     * @param maxLengthNumber   结果限制
     * @param label             标签
     ...
     *
     * @param columnName   列名称
     * @param jsAction     JS 动作
     * @param innerValue   内在价值
     * @param displayValue 显示值
     * @return {@link TextDisplayComponent }
     */
    public static ComboBox newComboBox(String columnName, String jsAction, ArrayList<String> innerValue, ArrayList<String> displayValue) {
        ComboBox comboBox = new ComboBox();
        comboBox.setInternalValues(innerValue);
        comboBox.setValues(displayValue);
        comboBox.setColumnName(columnName);
        comboBox.addJsAction("onchange", jsAction);
        comboBox.setId(columnName);
