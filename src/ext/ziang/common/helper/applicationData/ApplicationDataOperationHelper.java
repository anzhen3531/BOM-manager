...
public class ApplicationDataOperationHelper {
    ...
    private static final Logger log = LoggerFactory.getLogger(ApplicationDataOperationHelper.class);
    ...
    public static String downloadEpmDoc(String oid) throws WTException {
        ...
        if (epmDocument.getCADName().contains(".SLDDRW")) {
            ...
            try {
                QueryResult qr = ContentHelper.service.getContentsByRole(epmDocument, ContentRoleType.PRIMARY);
                if (qr == null) {
                    return null;
                }
                if (qr.hasMoreElements()) {
                    ApplicationData appData = (ApplicationData) qr.nextElement();
                    if (PersistenceHelper.isPersistent(appData)) {
                        String newPath = tempPath;
                        File file = new File(newPath);
                        if (!file.exists()) {
                            file.mkdirs();
                        }
                        String absoluteFileName = newPath + File.separator + epmDocument.getCADName();
